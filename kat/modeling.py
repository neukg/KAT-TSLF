...
from .gen_utils import GenerationMixinOld
...
class BartModel(PretrainedBartModel):
    ...
    def add_knowl_pos(self, knowl, num_kno):
        """
        ...
        # (bsz, num_kno, kno_len, 1)
        # coef = self.gatev(self.gatew(torch.cat([knowl, utter], dim=-1)).tanh()).sigmoid()
    ...
    def forward(
        ...
        **kwargs,
        ...
        if encoder_outputs is None:
            ...
            # (bsz, d_model)
            kno_last_hidden_state = self.add_knowl_pos(knowl_outputs["last_hidden_state"], max_num_knowl)
...
class BartForConditionalGeneration(PretrainedBartModel, GenerationMixinOld):
    ...
    def forward(
        ...
        **unused,
        ...
        #     use_cache = False
        #     if decoder_input_ids is None:
        #         decoder_input_ids, labels = self.modify_inp_and_tgt(labels, self.config.pad_token_id, self.config.eos_token_id)
            # if decoder_attention_mask is not None:
            #     decoder_attention_mask = self.modify_tgt_mask(decoder_attention_mask)
        # print(decoder_input_ids.shape, labels.shape)
        # print(decoder_input_ids[0].tolist(), labels[0].tolist())
                # decoder_input_ids = shift_tokens_right(labels, self.config.pad_token_id)
        # print("before into self.model:", encoder_outputs[0].last_hidden_state.shape, encoder_outputs[1].last_hidden_state.shape)
        # logger.info(f"{decoder_input_ids.shape}, {labels.shape}")
    ...
    def generate(
        ...
        **model_kwargs
        ...
        if self.config.is_encoder_decoder:
            ...
            encoder_outputs[1]["last_hidden_state"] = self.model.add_knowl_pos(encoder_outputs[1]["last_hidden_state"], max_num_knowl)
        ...
        if num_return_sequences > 1 or num_beams > 1:
            ...
            kno_attn_mask = kno_attn_mask.unsqueeze(1).expand(
                batch_size, effective_batch_mult * num_beams, num_know, knowl_ids_len
            )

            src_ids = src_ids.contiguous().view(
                effective_batch_size * num_beams, src_ids_len
            )  # shape: (batch_size * num_return_sequences * num_beams, cur_len)
            kno_ids = kno_ids.contiguous().view(
                effective_batch_size * num_beams * num_know, knowl_ids_len
            )  # shape: (batch_size * num_return_sequences * num_beams * num_know, cur_len)

            src_attn_mask = src_attn_mask.contiguous().view(
                effective_batch_size * num_beams, src_ids_len
            )  # shape: (batch_size * num_return_sequences * num_beams, cur_len)
            kno_attn_mask = kno_attn_mask.contiguous().view(
                effective_batch_size * num_beams, num_know, knowl_ids_len
            )  # shape: (batch_size * num_return_sequences * num_beams * num_know, cur_len)
        ...
        if self.config.is_encoder_decoder:
            ...
            assert (
                batch_size == encoder_outputs[0].last_hidden_state.shape[0]
            ...
            encoder_outputs[1]["last_hidden_state"] = tmp.index_select(
                0, expanded_batch_idxs
            ).view(-1, max_knowl_len, hidden_size)
            # print(encoder_outputs[0]["last_hidden_state"].shape, encoder_outputs[1]["last_hidden_state"].shape)
            # assert 0
